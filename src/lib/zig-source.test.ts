...
test('tui process exit stays outside managed resource scope', () => {
  ...
  assert.match(runMainBody, /std\.heap\.DebugAllocator/);
  assert.match(runMainBody, /defer std\.debug\.assert\(debug_allocator\.deinit\(\) == \.ok\);/);
  assert.match(runMainBody, /defer threaded\.deinit\(\);/);
