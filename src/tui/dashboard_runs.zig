...
fn isFailedRun(latest: JsonObject, field_name: []const u8) bool {
    ...
    const status = dashboard_json.stringField(run, "status", missing_status);
    if (!std.mem.eql(u8, status, completed_status)) return false;
    return !std.mem.eql(u8, dashboard_json.stringField(run, "conclusion", ""), success_conclusion);
}
...
fn runLabel(latest: JsonObject, field_name: []const u8) []const u8 {
    ...
    return dashboard_json.stringField(run, "conclusion", completed_status);
}
